...
export interface TextChunk {
  ...
  startPct: number;
  endPct: number;
  isOverlap: boolean; // true if this chunk is just overlap context
}

export interface ChunkOptions {
  /** Target chunk size in characters (default: ~60K chars ≈ 15K tokens) */
  targetChunkSize?: number;
  /** Number of paragraphs to overlap between chunks (default: 2) */
  overlapParagraphs?: number;
  /** Custom chapter marker regexes */
...
const DEFAULT_CHAPTER_MARKERS: RegExp[] = [
 ...
 */
function findChapterBoundaries(text: string, markers: RegExp[]): number[] {
 ...
 */
function extractOverlap(text: string, position: number, paraCount: number, direction: 'forward' | 'backward'): string {
 ...
 */
function extractOverlapFromParagraphs(paragraphs: string[], count: number): string {
