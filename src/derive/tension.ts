import { StoryModel, TextModel, Reading, Timestamp } from '../types';
...
function checkAbsentialQuality(textModel: TextModel): boolean {
  const absentials = Object.values(textModel.absentials);
  if (absentials.length === 0) return false;
  ...
  for (const abs of absentials) {
    if (abs.stateHistory.length >= 2) {
      ...
      if (new Set(pcts).size >= 2) multiState++;
    }
  }
 ...
 */
function computeFromAbsentials(
  textModel: TextModel,
  reading: Reading,
  steps: number,
): TensionPoint[] {
 ...
 */
function computeFromEventSignificance(
  textModel: TextModel,
  reading: Reading,
  steps: number,
): TensionPoint[] {
  // Collect events with their timestamps and significance
  const events: Array<{ pct: number; significance: number }> = [];
 ...
 */
function estimateEventPosition(evtId: string, textModel: TextModel): number {
  const allIds = Object.keys(textModel.events);
  const idx = allIds.indexOf(evtId);
  if (idx === -1 || allIds.length <= 1) return 50;
  return (idx / (allIds.length - 1)) * 100;
}

function normalize(points: TensionPoint[]): TensionPoint[] {
