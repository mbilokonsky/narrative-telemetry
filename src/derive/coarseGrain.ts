import { StorySpan, Reading, SpanID } from '../types';

export interface SpanSignificance {
  spanId: SpanID;
  title: string;
  type: string;
  meanSignificance: number;
  eventCount: number;
  children: SpanSignificance[];
}
 ...
 */
export function coarseGrain(rootSpan: StorySpan, reading: Reading): SpanSignificance {
  return computeSpanSignificance(rootSpan, reading);
}

function computeSpanSignificance(span: StorySpan, reading: Reading): SpanSignificance {
  ...
  const directSigs: number[] = span.events
    .map(eid => reading.eventSignificance[eid]?.significance)
    .filter((s): s is number => s !== undefined);

  let meanSignificance: number;
