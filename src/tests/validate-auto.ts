...
import { computeTensionCurve } from '../derive/tension';
import { computeDivergence } from '../derive/divergence';
import { coarseGrain } from '../derive/coarseGrain';
import { computePacing } from '../derive/pacing';
...
const model: StoryModel = JSON.parse(fs.readFileSync(outputPath, 'utf-8'));
...
function countSpansByType(span: StorySpan, type: string): number {
...
check(`No orphan events (${orphanEvents.length} orphaned)`, orphanEvents.length <= 3,
  orphanEvents.length > 3 ? `orphans: ${orphanEvents.slice(0, 5).join(', ')}` : undefined);
...
for (const [eid, evt] of Object.entries(model.text.events)) {
  for (const pid of (evt.participants || [])) {
    if (!allEntityIds.has(pid)) badRefs++;
  }
}
...
for (const name of readingNames) {
  ...
  check(`  Events annotated: ${annotatedEvents}/${eventCount}`, annotatedEvents >= eventCount * 0.8,
    `only ${((annotatedEvents/eventCount)*100).toFixed(0)}% coverage`);
  
  // Check significance scores are in valid range
  let badScores = 0;
  for (const [evtId, ann] of Object.entries(reading.eventSignificance || {})) {
    if (ann.significance < 0 || ann.significance > 1) badScores++;
  }
...
for (const name of readingNames) {
  const reading = model.readings[name];
  const curve = computeTensionCurve(model.text, reading, 20);
  const peak = curve.reduce((m, p) => p.tension > m.tension ? p : m, curve[0]);
  
  check(`Tension [${name}]: peak at ${peak.timestamp.percentage.toFixed(0)}% (target 50-90%)`, 
    inRange(peak.timestamp.percentage, 50, 90));
  check(`Tension [${name}]: curve has ${curve.length} points`, curve.length >= 10);
  check(`Tension [${name}]: max tension is 1.0 (normalized)`, 
    Math.abs(peak.tension - 1.0) < 0.01);
}
...
if (readingNames.length >= 2) {
  ...
  const div = computeDivergence(r1, r2);
  
  check(`Divergence: ${div.divergentEventCount} events with |diff| > 0.2 (target >= 5)`, 
    div.divergentEventCount >= 5);
  check(`Divergence: mean = ${div.meanEventDivergence.toFixed(3)} (should be > 0)`, 
    div.meanEventDivergence > 0);
  check(`Divergence: max = ${div.maxEventDivergence.toFixed(3)} (should be > 0.2)`, 
    div.maxEventDivergence > 0.2);
}

// Coarse-graining
for (const name of readingNames) {
  const reading = model.readings[name];
  const cg = coarseGrain(model.text.rootSpan, reading);
