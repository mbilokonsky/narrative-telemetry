...
import { StoryModel, StorySpan, Reading, TextModel } from '../types';
import { NarrativeEventType, StorySpanType } from '../types/core';
...
function buildTestModel(): StoryModel {
  const events: Record<string, Event> = {
    ...
    'evt-4': {
      id: 'evt-4',
      type: NarrativeEventType.DECISION,
  ...
  const reading: Reading = {
    ...
    narrator: { id: 'narrator', name: 'Narrator', description: '', tags: [], stateHistory: [], firstIntroduced: 'evt-1' } as any,
    reader: { id: 'reader', name: 'Reader', description: '', tags: [], stateHistory: [], firstIntroduced: 'evt-1' } as any,
    author: { id: 'author', name: 'Author', description: '', tags: [], stateHistory: [], firstIntroduced: 'evt-1' } as any,
    eventSignificance: {
...
const traceId = generateTraceId('Test Story', 'Test Author');
check('Trace ID is 32 hex chars (16 bytes)', traceId.length === 32 && /^[0-9a-f]+$/.test(traceId));
check('Trace ID is reproducible', traceId === generateTraceId('Test Story', 'Test Author'));
check('Different input → different trace ID', traceId !== generateTraceId('Other', 'Author'));
...
check('Revelation event has type attribute',
  revelationEvent?.attributes.some(a => a.key === 'narrative.event.type') ?? false);
check('Revelation event has participants',
  revelationEvent?.attributes.some(a => a.key === 'narrative.event.participants') ?? false);

// ── 4. Timestamp conversion ──

console.log('\n=== 4. Timestamp Conversion ===\n');

const baseNs = BigInt(BASE_TIME) * 1_000_000_000n;
const durationNs = BigInt(DURATION) * 1_000_000_000n;

// Root span: 0% → baseTime, 100% → baseTime + duration
check('Root span starts at baseTime',
  rootOtelSpan ? rootOtelSpan.startTimeUnixNano === baseNs.toString() : false);
check('Root span ends at baseTime + duration',
  rootOtelSpan ? rootOtelSpan.endTimeUnixNano === (baseNs + durationNs).toString() : false);
...
check('50% event timestamp correct',
  midEvent ? midEvent.timeUnixNano === expectedMidNs.toString() : false,
  midEvent ? `got ${midEvent.timeUnixNano}, expected ${expectedMidNs.toString()}` : 'event not found');

// 80% event
const expected80Ns = baseNs + (durationNs * 800n / 1000n);
const decisionEvent = allOtelEvents.find(e => e.name === 'Hero makes final choice');
check('80% event timestamp correct',
  decisionEvent ? decisionEvent.timeUnixNano === expected80Ns.toString() : false);
...
const readingAttrs = readingToAttributes(reading);

check('Reading name attribute present',
  readingAttrs.some(a => a.key === 'narrative.reading.name'));
check('Themes array present',
  readingAttrs.some(a => a.key === 'narrative.reading.themes'));
check('Symbols array present',
  readingAttrs.some(a => a.key === 'narrative.reading.symbols'));
check('Peak tension present',
  readingAttrs.some(a => a.key === 'narrative.reading.peak_tension'));

// Reading attributes on root span
const rootAttrs = rootOtelSpan?.attributes ?? [];
check('Root span has reading.name',
  rootAttrs.some(a => a.key === 'narrative.reading.name'));
check('Root span has reading.themes',
  rootAttrs.some(a => a.key === 'narrative.reading.themes'));
...
check('Climax event has significance = 1.0',
  sigAttr && 'doubleValue' in sigAttr.value ? sigAttr.value.doubleValue === 1.0 :
...
check('Act 3 has tension annotation',
  act3Span?.attributes.some(a => a.key === 'narrative.reading.tension') ?? false);

// ── 6. Span ID generation ──

console.log('\n=== 6. Span ID Generation ===\n');

const spanId1 = generateSpanId(traceId, 'story');
check('Span ID is 16 hex chars (8 bytes)', spanId1.length === 16 && /^[0-9a-f]+$/.test(spanId1));
check('Span ID is reproducible', spanId1 === generateSpanId(traceId, 'story'));
check('Different span → different ID', spanId1 !== generateSpanId(traceId, 'act-1'));
...
const jaegerJson = jaegerExport(trace);
let jaegerParsed: any;
try {
...
if (jaegerParsed) {
  ...
  const spanWithLogs = data.spans.find((s: any) => s.logs && s.logs.length > 0);
  check('At least one Jaeger span has logs (events)', !!spanWithLogs);

  // Check references
  const childSpan = data.spans.find((s: any) => s.references && s.references.length > 0);
  check('Child spans have CHILD_OF references', childSpan?.references[0]?.refType === 'CHILD_OF');

  // Timestamps should be in microseconds
  const rootJaeger = data.spans.find((s: any) => s.operationName.startsWith('story:'));
  if (rootJaeger) {
    const expectedStartMicro = BASE_TIME * 1_000_000;
...
if (otlpParsed) {
  ...
  check('OTLP resource has service.name', rs.resource?.attributes?.some((a: any) => a.key === 'service.name'));
...
const noReadingModel: StoryModel = {
  text: model.text,
  readings: {},
};
const noReadingTrace = storyModelToOtel(noReadingModel, { baseTime: BASE_TIME });
check('Export works with no readings', noReadingTrace.spans.length === 9);
check('Root span has no reading attributes when none available',
  !noReadingTrace.spans[0].attributes.some(a => a.key === 'narrative.reading.name'));
...
const trace1 = storyModelToOtel(multiModel, { baseTime: BASE_TIME, readingName: 'test-reading' });
const trace2 = storyModelToOtel(multiModel, { baseTime: BASE_TIME, readingName: 'second-reading' });
...
check('Different readings produce different significance values',
  !!(t1Sig && t2Sig && JSON.stringify(t1Sig.value) !== JSON.stringify(t2Sig.value)));
