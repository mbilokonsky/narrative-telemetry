...
function makeReading(eventSigs = {}) {
...
function makeTextModel(overrides = {}) {
    return {
        title: 'Test Story',
        author: 'Test Author',
        ...
        rootSpan: makeSpan({
            childSpans: [
                makeSpan({
                    ...
                    childSpans: [
                        makeSpan({
                            ...
                            childSpans: [
                                makeSpan({
                                    id: 'beat-1', type: types_1.StorySpanType.BEAT, title: 'Beat 1',
                                    startTimestamp: { percentage: 0 }, endTimestamp: { percentage: 12 },
                                    events: ['e01'],
                                }),
                                makeSpan({
                                    id: 'beat-2', type: types_1.StorySpanType.BEAT, title: 'Beat 2',
                        ...
                        makeSpan({
                            ...
                            childSpans: [
                                makeSpan({
                                    id: 'beat-3', type: types_1.StorySpanType.BEAT, title: 'Beat 3',
                                    startTimestamp: { percentage: 25 }, endTimestamp: { percentage: 50 },
                                    events: ['e03'],
                                }),
                            ],
                ...
                makeSpan({
                    id: 'act-2', type: types_1.StorySpanType.ACT, title: 'Act 2',
                    startTimestamp: { percentage: 50 }, endTimestamp: { percentage: 100 },
                    childSpans: [
                        makeSpan({
                            id: 'scene-3', type: types_1.StorySpanType.SCENE, title: 'Scene 3',
                            startTimestamp: { percentage: 50 }, endTimestamp: { percentage: 100 },
                            events: ['e04', 'e05'],
                            childSpans: [
                                makeSpan({
                                    id: 'beat-4', type: types_1.StorySpanType.BEAT, title: 'Beat 4',
                                    startTimestamp: { percentage: 50 }, endTimestamp: { percentage: 75 },
                                    events: ['e04'],
                                }),
                                makeSpan({
                                    id: 'beat-5', type: types_1.StorySpanType.BEAT, title: 'Beat 5',
        ...
        diegetic: {
            characters: {
                'char-1': { id: 'char-1', name: 'Alice', description: 'Protagonist', tags: [], isDiegetic: true, stateHistory: [], textMentions: [] },
                'char-2': { id: 'char-2', name: 'Bob', description: 'Antagonist', tags: [], isDiegetic: true, stateHistory: [], textMentions: [] },
            },
            settings: {
                'setting-1': { id: 'setting-1', name: 'Forest', description: 'Dark forest', tags: [], isDiegetic: true, stateHistory: [], textMentions: [] },
            },
            items: {},
            factions: {},
        },
        events: {
            'e01': { id: 'e01', type: types_1.NarrativeEventType.ACTION, description: 'Alice enters', timestamp: { percentage: 5 }, participants: ['char-1'], realm: 'material_reality', consequences: [] },
            'e02': { id: 'e02', type: types_1.NarrativeEventType.DIALOGUE, description: 'Alice speaks', timestamp: { percentage: 15 }, participants: ['char-1'], realm: 'material_reality', consequences: [] },
            'e03': { id: 'e03', type: types_1.NarrativeEventType.REVELATION, description: 'Secret revealed', timestamp: { percentage: 35 }, participants: ['char-1', 'char-2'], realm: 'material_reality', consequences: [] },
            'e04': { id: 'e04', type: types_1.NarrativeEventType.DECISION, description: 'Alice decides', timestamp: { percentage: 60 }, participants: ['char-1'], realm: 'material_reality', consequences: [] },
            'e05': { id: 'e05', type: types_1.NarrativeEventType.ACTION, description: 'Final confrontation', timestamp: { percentage: 85 }, participants: ['char-1', 'char-2'], realm: 'material_reality', consequences: [] },
        },
...
const curve = (0, tension_1.computeTensionCurve)(tm, reading1, 10);
...
check('Peak is in second half (climax zone)', curve.reduce((m, p) => p.tension > m.tension ? p : m, curve[0]).timestamp.percentage >= 50);
// Test with empty events
const emptyReading = makeReading({});
const emptyCurve = (0, tension_1.computeTensionCurve)(tm, emptyReading, 10);
check('Empty reading: returns points', emptyCurve.length > 0);
// ═══════════════════════════════════════════════
section('Coarse-Graining');
// ═══════════════════════════════════════════════
const cg = (0, coarseGrain_1.coarseGrain)(tm.rootSpan, reading1);
...
section('Pacing');
// ═══════════════════════════════════════════════
const pacing = (0, pacing_1.computePacing)(tm.rootSpan, tm);
...
const div = (0, divergence_1.computeDivergence)(reading1, reading2);
...
check('e02 divergence = |0.3 - 0.8| = 0.5', e02div !== undefined && Math.abs(e02div.diff - 0.5) < 0.01);
const e03div = div.events.find(e => e.eventId === 'e03');
check('e03 divergence = |0.7 - 0.3| = 0.4', e03div !== undefined && Math.abs(e03div.diff - 0.4) < 0.01);
// Top divergent events should be ordered
check('Top divergent events sorted by diff desc', div.topDivergentEvents.length > 0 &&
...
const selfDiv = (0, divergence_1.computeDivergence)(reading1, reading1);
...
// ═══════════════════════════════════════════════
const reg = new registry_1.EntityRegistry();
// Empty registry
...
// ═══════════════════════════════════════════════
// Empty text
const emptyChunks = (0, chunker_1.chunkText)('');
check('Empty text: returns 1 chunk', emptyChunks.length === 1);
// Single paragraph
const singleChunks = (0, chunker_1.chunkText)('Just one paragraph.');
...
const shortSceneChunks = (0, chunker_1.chunkText)(shortScene);
...
const romanChunks = (0, chunker_1.chunkText)(romanText, { targetChunkSize: 15000 });
check('Roman numeral chapters detected', romanChunks.filter(c => !c.isOverlap).length >= 2);
// Token estimation
check('Token estimate: 0 chars = 0 tokens', (0, chunker_1.estimateTokens)(0) === 0);
check('Token estimate: 100 chars = 25 tokens', (0, chunker_1.estimateTokens)(100) === 25);
check('Token estimate: 1 char = 1 token (ceiling)', (0, chunker_1.estimateTokens)(1) === 1);
// getChunkStats
const statsChunks = (0, chunker_1.chunkText)('test');
const stats = (0, chunker_1.getChunkStats)(statsChunks);
...
check('Events reference valid characters', tm.events['e03'].participants.every((p) => p in tm.diegetic.characters));
// ═══════════════════════════════════════════════
//  Summary
// ═══════════════════════════════════════════════
console.log(`  (${sectionPassed}/${sectionPassed + sectionFailed} passed)\n`);
