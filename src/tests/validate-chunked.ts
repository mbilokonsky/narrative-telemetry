...
import { chunkText, getChunkStats, estimateTokens } from '../ingest/chunker';
...
check('Short text: correct percentages', 
  shortChunks[0].startPct === 0 && shortChunks[0].endPct === 100);
...
const chapterChunks = chunkText(chapterText, { targetChunkSize: 10000 });
...
const sceneChunks = chunkText(sceneText, { targetChunkSize: 5000 });
...
const longChunks = chunkText(paragraphs, { targetChunkSize: 15000 });
check('Long text: splits into multiple chunks', longChunks.filter(c => !c.isOverlap).length > 1 || longChunks[0].text.length > 15000);
check('Long text: chunks have correct percentages', 
  longChunks.filter(c => !c.isOverlap).length > 0 && longChunks[longChunks.length - 1].endPct === 100);
...
check('Token estimation: ~4 chars/token', 
  estimateTokens(testChars) >= 900 && estimateTokens(testChars) <= 1100);
...
const chunk1Registry = new EntityRegistry();
...
const chunk2Registry = EntityRegistry.fromJSON(chunk1Registry.toJSON());
...
const restoredRegistry = EntityRegistry.fromJSON(jsonData);
