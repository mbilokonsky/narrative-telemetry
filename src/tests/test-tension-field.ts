...
import { computeTensionField, flattenTensionField, TensionField, TensionCurvePoint } from '../derive/tensionField';
import { StoryModel, StorySpanType, NarrativeEventType, TextModel, Reading } from '../types';
...
const minimalModel: TextModel = {
  title: 'Minimal', author: 'Test', description: 'Test',
  rootSpan: {
    id: 'root', type: StorySpanType.STORY, title: 'Root',
  ...
  diegetic: {
    characters: {
      'alice': { id: 'alice', name: 'Alice', description: 'Protagonist', tags: [], isDiegetic: true, stateHistory: [], textMentions: [] } as any,
      'bob': { id: 'bob', name: 'Bob', description: 'Antagonist', tags: [], isDiegetic: true, stateHistory: [], textMentions: [] } as any,
    },
    settings: {
      'forest': { id: 'forest', name: 'Dark Forest', description: 'A dark place', tags: [], isDiegetic: true, stateHistory: [], textMentions: [] } as any,
    },
    items: {}, factions: {},
  },
  events: {
    'e1': { id: 'e1', type: NarrativeEventType.ACTION, description: 'Alice enters forest', timestamp: { percentage: 10 }, participants: ['alice', 'forest'], realm: 'material_reality', consequences: [] } as any,
    'e2': { id: 'e2', type: NarrativeEventType.DIALOGUE, description: 'Alice meets Bob', timestamp: { percentage: 30 }, participants: ['alice', 'bob'], realm: 'material_reality', consequences: [] } as any,
    'e3': { id: 'e3', type: NarrativeEventType.REVELATION, description: 'Bob reveals secret', timestamp: { percentage: 70 }, participants: ['alice', 'bob'], realm: 'material_reality', consequences: [] } as any,
  },
  relationships: {
    interpersonal: {
      'rel-1': {
...
const minimalReading: Reading = {
  name: 'test', description: 'Test reading',
  themes: {}, symbols: {}, symbolicRelationships: {},
  narrator: {} as any, reader: {} as any, author: {} as any,
  eventSignificance: {
    'e1': { significance: 0.3 },
    'e2': { significance: 0.6 },
    'e3': { significance: 0.9 },
...
const field = computeTensionField(minimalModel, minimalReading);
...
const aliceBobInteraction = scene1.interactions.find(
  i => (i.entity1 === 'alice' && i.entity2 === 'bob') ||
...
check('All dimensions between 0 and 1',
  Object.values(scene1.fieldDimensions).every(v => v >= 0 && v <= 1));

// ═══════════════════════════════════════════════
console.log('\n--- Flatten to Curves ---\n');
// ═══════════════════════════════════════════════

const curves = flattenTensionField(field, 10);
...
const hasNonZero = curves.some(p =>
  p.absential > 0 || p.relational > 0 || p.epistemic > 0 || p.atmospheric > 0 || p.pacing > 0
);
...
if (fs.existsSync(arabyPath)) {
  const arabyModel: StoryModel = JSON.parse(fs.readFileSync(arabyPath, 'utf-8'));
  const readingNames = Object.keys(arabyModel.readings);

  if (readingNames.length > 0) {
    const arabyField = computeTensionField(arabyModel.text, arabyModel.readings[readingNames[0]]);
    check('Araby field computed', arabyField !== undefined);
    check('Araby field has children', arabyField.children.length > 0);

    const arabyCurves = flattenTensionField(arabyField);
    check('Araby curves computed', arabyCurves.length > 0);
    check('Araby has multi-dimensional tension',
      arabyCurves.some(p => p.absential > 0 || p.relational > 0 || p.epistemic > 0));
    ...
    check('Araby composite peaks somewhere reasonable',
      peak.timestamp.percentage >= 20 && peak.timestamp.percentage <= 90);

    // If we have two readings, compare their tension fields
    if (readingNames.length >= 2) {
      const field1 = computeTensionField(arabyModel.text, arabyModel.readings[readingNames[0]]);
      const field2 = computeTensionField(arabyModel.text, arabyModel.readings[readingNames[1]]);
      const curves1 = flattenTensionField(field1);
      const curves2 = flattenTensionField(field2);
      ...
      const diff = curves1.reduce((sum, p, i) =>
        sum + Math.abs(p.absential - curves2[i].absential) +
              ...
              Math.abs(p.composite - curves2[i].composite),
      0);
