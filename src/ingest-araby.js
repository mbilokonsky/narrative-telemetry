...
function loc(start, end) { return { startLine: start, endLine: end }; }
...
// ════════════════════════════════════════════════════════
const sys = new NarrativeAnalysisSystem_1.NarrativeAnalysisSystem('Araby', 'James Joyce', 'A boy in Dublin becomes infatuated with his friend\'s sister and promises to bring her something from the Araby bazaar, only to arrive too late and experience a moment of devastating self-awareness.');
...
const northRichmondSt = sys.addSetting({
    ...
    tags: ['dublin', 'residential', 'dead-end'], type: types_1.DiegeticEntityType.SETTING,
...
const house = sys.addSetting({
    ...
    tags: ['domestic', 'gloomy', 'religious-remnants'], type: types_1.DiegeticEntityType.SETTING,
...
const backDrawingRoom = sys.addSetting({
    ...
    tags: ['domestic', 'death', 'devotion'], type: types_1.DiegeticEntityType.SETTING,
...
const wildGarden = sys.addSetting({
    ...
    tags: ['garden', 'decay', 'nature'], type: types_1.DiegeticEntityType.SETTING,
...
const marketStreets = sys.addSetting({
    ...
    tags: ['commercial', 'sensory', 'hostile'], type: types_1.DiegeticEntityType.SETTING,
...
const classroom = sys.addSetting({
    ...
    tags: ['school', 'constraint', 'distraction'], type: types_1.DiegeticEntityType.SETTING,
...
const buckinghamSt = sys.addSetting({
    ...
    tags: ['transit', 'journey'], type: types_1.DiegeticEntityType.SETTING,
...
const trainCarriage = sys.addSetting({
    ...
    tags: ['transit', 'isolation'], type: types_1.DiegeticEntityType.SETTING,
...
const arabyHall = sys.addSetting({
    ...
    tags: ['bazaar', 'exotic', 'commercial', 'disillusionment'], type: types_1.DiegeticEntityType.SETTING,
...
sys.addInterpersonalRelationship({
    id: 'rel-boy-mangans-sister', type: types_1.RelationshipType.INTERPERSONAL,
...
sys.addInterpersonalRelationship({
    id: 'rel-boy-uncle', type: types_1.RelationshipType.INTERPERSONAL,
...
sys.addInterpersonalRelationship({
    id: 'rel-boy-mangan', type: types_1.RelationshipType.INTERPERSONAL,
...
sys.addInterpersonalRelationship({
    id: 'rel-uncle-aunt', type: types_1.RelationshipType.INTERPERSONAL,
...
const absQuest = sys.addAbsential({
    ...
    parentAbsential: absLonging, childAbsentials: [], conflictingAbsentials: [],
    relatedEntities: [
        { entityId: mangansSister, relationship: types_1.EntityAbsentialRelationship.BENEFICIARY, strength: 0.8 },
        { entityId: arabyHall, relationship: types_1.EntityAbsentialRelationship.TARGET, strength: 0.7 },
...
// ── Span structure ──
const act1 = sys.createSpan(rootId, types_1.StorySpanType.ACT, 'Act 1: The Street and the Infatuation', 'Setting, characters, the boy\'s growing obsession', 0, 34);
const act2 = sys.createSpan(rootId, types_1.StorySpanType.ACT, 'Act 2: The Promise and the Waiting', 'The conversation, the agonizing wait', 34, 72);
const act3 = sys.createSpan(rootId, types_1.StorySpanType.ACT, 'Act 3: Araby and the Epiphany', 'The journey and devastating realization', 72, 100);
const s1a = sys.createSpan(act1, types_1.StorySpanType.SCENE, 'North Richmond Street described', 'The blind street, the dead priest, the house', 0, 9);
const s1b = sys.createSpan(act1, types_1.StorySpanType.SCENE, 'Street play and first sightings', 'Winter evenings, play, watching Mangan\'s sister', 9, 18);
const s1c = sys.createSpan(act1, types_1.StorySpanType.SCENE, 'The daily ritual of following', 'Morning watching, following her through the streets', 18, 23);
const s1d = sys.createSpan(act1, types_1.StorySpanType.SCENE, 'The chalice passage', 'Her image in hostile places, the market, the chalice metaphor', 23, 30);
const s1e = sys.createSpan(act1, types_1.StorySpanType.SCENE, 'The back drawing-room', '"O love! O love!" in the room where the priest died', 30, 34);
const s2a = sys.createSpan(act2, types_1.StorySpanType.SCENE, 'The conversation at the railing', 'First real exchange, Araby mentioned, the promise', 34, 42);
const s2b = sys.createSpan(act2, types_1.StorySpanType.SCENE, 'Days of distraction', 'School, enchantment, the aunt\'s Freemasonry comment', 42, 50);
const s2c = sys.createSpan(act2, types_1.StorySpanType.SCENE, 'Saturday — waiting for the uncle', 'Clock-watching, Mrs. Mercer, the uncle arrives late', 50, 68);
const s2d = sys.createSpan(act2, types_1.StorySpanType.SCENE, 'The uncle gives money', 'Uncle drunk, forgotten, aunt intervenes, Arab\'s Farewell', 68, 72);
const s3a = sys.createSpan(act3, types_1.StorySpanType.SCENE, 'The journey', 'Buckingham Street, the train ride', 72, 80);
const s3b = sys.createSpan(act3, types_1.StorySpanType.SCENE, 'Arriving at the bazaar', 'Turnstile, the dark hall, church-like silence, coins on salver', 80, 86);
const s3c = sys.createSpan(act3, types_1.StorySpanType.SCENE, 'The stall and the flirtation', 'Overhearing the banal English flirtation, the young lady\'s question', 86, 95);
const s3d = sys.createSpan(act3, types_1.StorySpanType.SCENE, 'The epiphany', 'The lights go out, the boy sees himself clearly', 95, 100);
// ── Events ──
const e01 = sys.addEvent(s1a, { id: 'e01-street-described', type: types_1.NarrativeEventType.ENVIRONMENTAL, description: 'North Richmond Street described as blind, quiet except when the Christian Brothers\' School sets boys free. Uninhabited house at the blind end.', timestamp: ts(1), textLocation: loc(4, 9), participants: [northRichmondSt] });
const e02 = sys.addEvent(s1a, { id: 'e02-priest-legacy', type: types_1.NarrativeEventType.ENVIRONMENTAL, description: 'The dead priest\'s legacy: musty air, waste room littered with papers, three books found (The Abbot, The Devout Communicant, The Memoirs of Vidocq). Rusty bicycle-pump in the wild garden.', timestamp: ts(3), textLocation: loc(11, 21), participants: [deadPriest, house, bookAbbot, bookDevout, bookVidocq, bicyclePump, wildGarden], precedingEvent: 'e01-street-described' });
const e03 = sys.addEvent(s1b, { id: 'e03-winter-play', type: types_1.NarrativeEventType.ACTION, description: 'Winter evenings: the boys play in the dark streets, running through muddy lanes, past dark gardens and odorous stables. If the uncle is seen they hide; if Mangan\'s sister appears they watch from the shadow.', timestamp: ts(10), textLocation: loc(23, 43), participants: [boy, mangan, uncle, mangansSister, northRichmondSt], precedingEvent: 'e02-priest-legacy' });
const e04 = sys.addEvent(s1b, { id: 'e04-sister-appears', type: types_1.NarrativeEventType.ENVIRONMENTAL, description: 'Mangan\'s sister appears at the doorstep to call her brother. Her figure defined by the light from the half-opened door, dress swinging, soft rope of hair tossed from side to side.', timestamp: ts(14), textLocation: loc(36, 43), participants: [mangansSister, mangan, boy], precedingEvent: 'e03-winter-play' });
const e05 = sys.addEvent(s1c, { id: 'e05-daily-following', type: types_1.NarrativeEventType.ACTION, description: 'Every morning the boy lies on the floor in the front parlour watching her door through the blind, then follows her silently through the streets, quickening his pace to pass her.', timestamp: ts(19), textLocation: loc(45, 53), participants: [boy, mangansSister], precedingEvent: 'e04-sister-appears' });
const e06 = sys.addEvent(s1d, { id: 'e06-chalice-passage', type: types_1.NarrativeEventType.ACTION, description: 'Her image accompanies the boy to the Saturday market with his aunt. Amid drunken men, bargaining women, street-singers chanting about O\'Donovan Rossa. He imagines bearing his chalice safely through a throng of foes.', timestamp: ts(25), textLocation: loc(55, 63), participants: [boy, mangansSister, aunt, marketStreets], precedingEvent: 'e05-daily-following' });
const e07 = sys.addEvent(s1d, { id: 'e07-harp-metaphor', type: types_1.NarrativeEventType.REVELATION, description: 'Strange prayers and praises spring to the boy\'s lips. His eyes fill with tears he cannot explain. His body is like a harp and her words and gestures are like fingers running upon the wires.', timestamp: ts(28), textLocation: loc(63, 70), participants: [boy, mangansSister], precedingEvent: 'e06-chalice-passage' });
const e08 = sys.addEvent(s1e, { id: 'e08-o-love', type: types_1.NarrativeEventType.REVELATION, description: 'In the back drawing-room where the priest died, on a dark rainy evening, the boy presses his palms together and murmurs "O love! O love!" many times.', timestamp: ts(32), textLocation: loc(72, 80), participants: [boy, backDrawingRoom, deadPriest], precedingEvent: 'e07-harp-metaphor' });
const e09 = sys.addEvent(s2a, { id: 'e09-conversation', type: types_1.NarrativeEventType.DIALOGUE, description: 'At last she speaks to him. She asks if he is going to Araby. She cannot go because of a convent retreat. She turns a silver bracelet round and round her wrist. Light catches the white curve of her neck.', timestamp: ts(36), textLocation: loc(82, 97), participants: [boy, mangansSister, silverBracelet], precedingEvent: 'e08-o-love' });
sys.updateAbsentialStatus('abs-speak', types_1.AbsentialStatus.RESOLVED_SATISFIED, 'e09-conversation', ts(36));
const e10 = sys.addEvent(s2a, { id: 'e10-promise', type: types_1.NarrativeEventType.DIALOGUE, description: '"If I go," I said, "I will bring you something." The boy makes the promise.', timestamp: ts(40), textLocation: loc(99, 101), participants: [boy, mangansSister], precedingEvent: 'e09-conversation' });
const e11 = sys.addEvent(s2b, { id: 'e11-distraction', type: types_1.NarrativeEventType.ACTION, description: 'Days of distraction: the boy wishes to annihilate the intervening days, chafes against school. The syllables of "Araby" cast an Eastern enchantment. Her image comes between him and the page.', timestamp: ts(44), textLocation: loc(103, 108), participants: [boy, classroom, schoolmaster], precedingEvent: 'e10-promise' });
const e12 = sys.addEvent(s2b, { id: 'e12-freemasonry', type: types_1.NarrativeEventType.DIALOGUE, description: 'The boy asks for leave to go to the bazaar on Saturday night. His aunt is surprised and hopes it is not some Freemason affair.', timestamp: ts(46), textLocation: loc(109, 110), participants: [boy, aunt], precedingEvent: 'e11-distraction' });
const e13 = sys.addEvent(s2b, { id: 'e13-schoolmaster', type: types_1.NarrativeEventType.ACTION, description: 'The schoolmaster\'s face passes from amiability to sternness; he hopes the boy is not beginning to idle.', timestamp: ts(48), textLocation: loc(111, 112), participants: [schoolmaster, boy], precedingEvent: 'e12-freemasonry' });
const e14 = sys.addEvent(s2c, { id: 'e14-saturday-reminder', type: types_1.NarrativeEventType.DIALOGUE, description: 'Saturday morning: the boy reminds his uncle about the bazaar. "Yes, boy, I know." The uncle is fussing at the hallstand.', timestamp: ts(51), textLocation: loc(117, 122), participants: [boy, uncle], precedingEvent: 'e13-schoolmaster' });
const e15 = sys.addEvent(s2c, { id: 'e15-clock-watching', type: types_1.NarrativeEventType.ACTION, description: 'The boy sits staring at the clock. Mounts the staircase, sings in the empty rooms. From the front window watches companions playing, looks at the dark house where she lives, sees her brown-clad figure cast by imagination.', timestamp: ts(55), textLocation: loc(127, 137), participants: [boy, house, mangansSister], precedingEvent: 'e14-saturday-reminder' });
const e16 = sys.addEvent(s2c, { id: 'e16-mrs-mercer', type: types_1.NarrativeEventType.ACTION, description: 'Mrs. Mercer arrives, sits at the fire. The boy endures the gossip of the tea-table. The meal stretches beyond an hour. Mrs. Mercer leaves after eight o\'clock.', timestamp: ts(58), textLocation: loc(139, 145), participants: [mrsMercer, boy, aunt, house], precedingEvent: 'e15-clock-watching' });
const e17 = sys.addEvent(s2c, { id: 'e17-aunt-warning', type: types_1.NarrativeEventType.DIALOGUE, description: 'The aunt says: "I\'m afraid you may put off your bazaar for this night of Our Lord."', timestamp: ts(62), textLocation: loc(146, 148), participants: [aunt, boy], precedingEvent: 'e16-mrs-mercer' });
const e18 = sys.addEvent(s2d, { id: 'e18-uncle-arrives', type: types_1.NarrativeEventType.ACTION, description: 'At nine o\'clock the uncle\'s latchkey in the halldoor. He talks to himself, the hallstand rocks under his overcoat. The boy interprets these signs. The uncle has forgotten about the bazaar.', timestamp: ts(68), textLocation: loc(150, 154), participants: [uncle, boy, house], precedingEvent: 'e17-aunt-warning' });
const e19 = sys.addEvent(s2d, { id: 'e19-aunt-intervenes', type: types_1.NarrativeEventType.DIALOGUE, description: 'The aunt intervenes: "Can\'t you give him the money and let him go? You\'ve kept him late enough as it is." The uncle apologizes, quotes "All work and no play," asks about The Arab\'s Farewell to his Steed.', timestamp: ts(70), textLocation: loc(156, 168), participants: [aunt, uncle, boy, florin], precedingEvent: 'e18-uncle-arrives' });
const e20 = sys.addEvent(s3a, { id: 'e20-buckingham-st', type: types_1.NarrativeEventType.ACTION, description: 'The boy holds a florin tightly, strides down Buckingham Street. The streets thronged with buyers, glaring with gas. He takes a seat in a third-class carriage of a deserted train.', timestamp: ts(74), textLocation: loc(170, 178), participants: [boy, florin, buckinghamSt, trainCarriage], precedingEvent: 'e19-aunt-intervenes' });
const e21 = sys.addEvent(s3a, { id: 'e21-train-ride', type: types_1.NarrativeEventType.ACTION, description: 'After intolerable delay the train creeps among ruinous houses, over the twinkling river. At Westland Row a crowd is pressed back — special train for the bazaar. The boy remains alone.', timestamp: ts(77), textLocation: loc(174, 179), participants: [boy, trainCarriage], precedingEvent: 'e20-buckingham-st' });
const e22 = sys.addEvent(s3b, { id: 'e22-entering-bazaar', type: types_1.NarrativeEventType.ACTION, description: 'The boy passes through a turnstile, handing a shilling to a weary-looking man. He finds himself in a big hall girdled by a gallery. Nearly all stalls closed, greater part in darkness.', timestamp: ts(82), textLocation: loc(184, 189), participants: [boy, turnstileMan, arabyHall], precedingEvent: 'e21-train-ride' });
const e23 = sys.addEvent(s3b, { id: 'e23-church-silence', type: types_1.NarrativeEventType.ENVIRONMENTAL, description: 'A silence like that which pervades a church after a service. Two men counting money on a salver. The fall of the coins.', timestamp: ts(84), textLocation: loc(188, 193), participants: [boy, arabyHall], precedingEvent: 'e22-entering-bazaar' });
const e24 = sys.addEvent(s3c, { id: 'e24-examining-wares', type: types_1.NarrativeEventType.ACTION, description: 'Remembering with difficulty why he had come, the boy examines porcelain vases and flowered tea-sets at a stall.', timestamp: ts(87), textLocation: loc(195, 196), participants: [boy, arabyHall], precedingEvent: 'e23-church-silence' });
const e25 = sys.addEvent(s3c, { id: 'e25-english-flirtation', type: types_1.NarrativeEventType.DIALOGUE, description: 'A young lady is talking and laughing with two young gentlemen. The boy remarks their English accents. Banal flirtation: "O, I never said such a thing!" / "O, but you did!" / "O, there\'s a ... fib!"', timestamp: ts(89), textLocation: loc(197, 211), participants: [stallLady, twoGentlemen, boy], precedingEvent: 'e24-examining-wares' });
const e26 = sys.addEvent(s3c, { id: 'e26-no-thank-you', type: types_1.NarrativeEventType.DIALOGUE, description: 'The young lady comes over and asks if he wishes to buy anything. Her tone is not encouraging — she speaks out of a sense of duty. The boy murmurs "No, thank you."', timestamp: ts(91), textLocation: loc(213, 219), participants: [stallLady, boy], precedingEvent: 'e25-english-flirtation' });
const e27 = sys.addEvent(s3c, { id: 'e27-coins-falling', type: types_1.NarrativeEventType.ACTION, description: 'The boy lingers before her stall to make his interest seem real, then turns away. He allows the two pennies to fall against the sixpence in his pocket.', timestamp: ts(93), textLocation: loc(225, 228), participants: [boy, remainingCoins], precedingEvent: 'e26-no-thank-you' });
const e28 = sys.addEvent(s3d, { id: 'e28-epiphany', type: types_1.NarrativeEventType.REVELATION, description: 'A voice calls from the gallery that the light is out. The upper hall is completely dark. Gazing up into the darkness the boy sees himself as a creature driven and derided by vanity; his eyes burn with anguish and anger.', timestamp: ts(98), textLocation: loc(228, 233), participants: [boy, arabyHall], precedingEvent: 'e27-coins-falling' });
sys.updateAbsentialStatus('abs-quest', types_1.AbsentialStatus.RESOLVED_BLOCKED, 'e28-epiphany', ts(98));
sys.updateAbsentialStatus('abs-longing', types_1.AbsentialStatus.RESOLVED_MIXED, 'e28-epiphany', ts(98));
// ════════════════════════════════════════════════════════
...
for (const [pct, val] of [[0, 0.2], [10, 0.3], [20, 0.5], [28, 0.6], [32, 0.65], [36, 0.75], [40, 0.7], [45, 0.55], [51, 0.6], [55, 0.7], [62, 0.8], [68, 0.85], [74, 0.75], [82, 0.8], [89, 0.9], [93, 0.85], [98, 1.0]]) {
...
for (const [pct, val] of [[0, 0.3], [10, 0.2], [25, 0.5], [36, 0.4], [44, 0.6], [58, 0.3], [70, 0.7], [74, 0.6], [82, 0.7], [89, 1.0], [93, 0.9], [98, 0.95]]) {
...
const filePath = (0, persistence_1.saveStoryModel)(finalModel);
